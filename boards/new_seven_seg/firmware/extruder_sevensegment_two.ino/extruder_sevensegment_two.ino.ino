...
/*
Gnexlab extruder controller v5 - July 2018
WebSite: www.makerstorage.com
...
#define DISPLAY_ADDRESS   0x70 //#define DISPLAY_ADDRESS   0x71
...
boolean selectedSensor = 0; //default thermistor
...
long BUT_previousMillis = 0;        // will store last time button pressed
long BUT_interval = 1000;           // interval at which to click (milliseconds)
...
setSensor(); // will read the jumper and set the sensor
...
void setSensor() {
    // will read the jumper
    if (digitalRead(sensorSelectionPin) == HIGH) {
        selectedSensor = 0; // thermistor
      }  else {
        selectedSensor = 1; // thermocouple  
      }
...
void read_Temperatures(){
  ...
  if (selectedSensor) {
     ...
     Current_Extruder_Temp = read_termocouple();
  }  
